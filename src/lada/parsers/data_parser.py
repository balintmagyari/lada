...
class LammpsData:
    ...
    def to_pandas(
        ...
        if section == "Atoms":
            ...
            return pd.DataFrame(
                data_array, columns=["bond_id", "bond_type", "atom1_id", "atom2_id"]
            )
...
def read_data_file(filepath: str) -> LammpsData:
    ...
    sections = {
        name: np.loadtxt(data) if data else np.array([]) for name, data in sections_raw.items()
    }
...
def read_lammps_acf(filepath: str | Path, lag_col: str = "lag_time") -> pd.DataFrame:
    """
    ...
    if not records:
        raise ValueError(
            f"No valid data rows found in the last timestep block (Timestep: {timestep_value})."
        )
